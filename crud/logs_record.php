...
include "../crud/php/logs_connection.php";
	...
	if(isset($_GET['id']))
	{
		...
		$logs_query = $logs_conn->query("SELECT * FROM logs_". $id ." WHERE student_num='$id'") or die(mysqli_error());
		...
		<div class="body bg-dark mw-100 mt-2" style="height:70px; position: relative;">
			<nav style="float: left; margin-right: 20px;">
				<ul class="ul_" style="margin-top: 2%;">
					...
					<li class="lili"><?php echo "<a href='http://localhost/project/crud/logs_record.php?id={$id}' class='a_nav text-warning'>View Logs</a>"?></li>
				</ul>
			</nav>
		...
		<div class="body_1 mw-75" style="height: 500px; background: #F0F0F0;position: relative;">
			<table class="table table-striped table-dark">
				  ...
				  <tbody>
					...
					<?php
						while($logs_result = $logs_query->fetch_array())
						{
					?>

						<tr>
							<td><?php echo $logs_result['student_num'] ?></td>
							<td><?php echo $logs_result['date']."20"; ?></td>
							<td><?php echo $logs_result['time'] ?></td>
						</tr>
